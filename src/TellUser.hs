...
data TellUser
    ...
    | HereIsYourDailyChart (Either H.InternalError [(Int, Float)])
    | HereIsYourDailyMean (Either H.InternalError Int)
    | HereIsYourSummary (Either H.InternalError [(String, Int)])
                        P.ClockFileState
    | HereIsYourTagList [String]
    | HereIsYourTodayChart (Either H.InternalError [(String, Int)])
                           P.ClockFileState
    | HereIsYourTotal (Either H.InternalError Float)
...
instance Show TellUser where
    ...
    show (YouAreAlreadyClockedIn tags) =
        ...
        ":\n" ++ unwords tags
    show YouAreAlreadyClockedOut =
        "You are already clocked \
        \out."
    show YouCantSwitchWhenYoureClockedOut =
        "You are \
        \already clocked out, so you can't use the 'switch' \
        \option.  Perhaps you want to use 'clockin' instead."
    show YouCantSwitchToYourCurrentTask =
        "You are \
...
printSummaryChart c xs = zipWith makeString sortxs percents ++ [show c]
  where
    ...
    makeString (tag, dur) percent =
        ...
        "    " ++
        formatNum (i2f dur) longestNum ++ "    " ++ toNright maxPercents percent
...
makePercent 0 _ = "0%"
makePercent total num = show percent ++ "%"
  where
    percent :: Int
...
makeBarForGraphic (day, duration) =
    show day ++ formatNum (duration * 1000) 5 ++ ' ' : replicate barLen '#'

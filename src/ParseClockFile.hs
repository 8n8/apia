...
instance Show ClockFileState where
    ...
    show (LastClockOpen tags@[_]) = startString ++ " is:\n" ++ unwords tags
    show (LastClockOpen tags) = startString ++ "s are:\n" ++ unwords tags
    show AllClocksClosed = "All the clocks are closed."
    show Empty = "The clock file is empty."

startString :: String
startString = "The clock at the end of the file is open. It's tag"
...
instance Show BadLineType where
    show BeginTimeIsInTheFuture = "The start time is in the future."
    show EmptyLine = "The line is empty."
    show EndTimeIsBeforeBeginTime =
        "The closing clock is \
        \earlier than the opening clock."
    show EndTimeIsInTheFuture =
        "The closing clock is in \
        \the future."
    show LastWordNotNumeric =
        "The last word is not a \
...
file2sess ::
       Float -> String -> Either InternalParseError [Either BadLine Session]
file2sess now f =
    sequence [line2sess n ofN now line | (n, line) <- zip [1 ..] lns]
  where
...
line2sess n _ _ [] = Right (Left BadLine {errType = EmptyLine, lineNum = n})
line2sess n _ _ [_] =
    Right (Left BadLine {errType = OnlyOneWordOnLine, lineNum = n})
line2sess n ofN now line =
  ...
  where
    ...
    (start, stop) =
        if clockClosed
            then (s2f . last . init $ line, Closed (s2f $ last line))
...
data InternalParseError
    ...
    | ParseBackwardsGivenEmptyList
    deriving (Eq, Show)

-- It takes in a line and works out what is wrong with it,
-- if anything.
lookForBadWords :: [String] -> Either InternalParseError (Maybe BadLineType)
...
parseBackwards :: [WordType] -> Either InternalParseError (Maybe BadLineType)
...
parseBackwards [Clock, Clock] = Right (Just AtLeastOneNumericTag)
parseBackwards (Clock:Clock:xs)
    | all (Tag ==) xs = Right Nothing
    | otherwise = Right (Just AtLeastOneNumericTag)

-- It does some checks on the clocks in a session.
lookForBadClock :: Float -> ClockEnd -> Int -> Int -> Float -> Maybe BadLineType
